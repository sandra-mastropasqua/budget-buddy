...
class BudgetBuddyApp(ctk.CTk):
    ...
    def create_account(self):
        ...
        def submit_registration():
            """Soumet l'inscription et crée un compte bancaire."""
            ...
            if user_id:
                account_id = Account.create_account(user_id)  # Crée un compte pour l'utilisateur
                if account_id:
                    self.message_label.configure(text="Compte créé avec succès!", text_color="green")
                    register_window.destroy()
                else:
                    self.message_label.configure(text="Erreur lors de la création du compte bancaire", text_color="red")
            else:
                self.message_label.configure(text="Erreur lors de la création du compte utilisateur", text_color="red")
...
class Dashboard(ctk.CTk):
    def __init__(self, user_id):
        ...
        self.credit_button = ctk.CTkButton(self, text="Créditer 100€", command=self.credit)
        self.credit_button.pack(pady=5)
        
        self.debit_button = ctk.CTkButton(self, text="Débiter 50€", command=self.debit)
